...
which uses CPython 3.13 compiled to `wasm32-wasip1`.
...
| Aspect | Value |
|--------|-------|
| Interpreter | CPython 3.13 (C, cross-compiled to wasm32-wasip1) |
| FFI crate | `basilisk_cpython` |
...
The `basilisk_vm_value_derive` proc-macro crate emits CPython-specific derive implementations.
...
| Type | Description |
|---|---|
| `Interpreter` | CPython lifecycle and global namespace |
| `Scope` | Globals/locals dict for executing user code |
| `PyObjectRef` | RAII wrapper around `PyObject*` |
...
CPython uses a C extension module (`_basilisk_ic`) generated by `cpython_ic_object/`.
The Python-side `basilisk` module imports from `_basilisk_ic` transparently.
...
| File | Issue | Fix |
|------|-------|-----|
| `cpython_vm_value_conversion/try_into_vm_value_impls.rs` | Missing `BasiliskTryIntoVec` trait definition | Copied trait + all impls from `vec.rs` |
...
3. ~~Performance benchmarking~~ → DONE: CPython benchmarks in `benchmarks/counter/`
