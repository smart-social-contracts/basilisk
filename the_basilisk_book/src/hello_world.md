...
Let's build your first application (canister) with Basilisk!

Before embarking please ensure you've followed all of [the installation instructions](./installation.md).

We'll build a simple `Hello World` canister that shows the basics of importing Basilisk, exposing a query method, exposing an update method, and storing some state in a global variable. We'll then interact with it from the command line and from our web browser.
...
```bash
mkdir basilisk_hello_world
cd basilisk_hello_world
...
Open up `basilisk_hello_world` in your text editor (we recommend [VS Code](https://code.visualstudio.com/) with the [Microsoft Python extension](https://marketplace.visualstudio.com/items?itemName=ms-python.python)).

## main.py

Here's the main code of the project, which you should put in the `basilisk_hello_world/src/main.py` file of your canister:

```python
from basilisk import query, update, void
...
The code starts off by importing the `query` and `update` decorators from `basilisk`, along with the `void` type. The `basilisk` module provides most of the Internet Computer (IC) APIs for your canister.
...
Create the following in `basilisk_hello_world/dfx.json`:

```json
{
    "canisters": {
        "basilisk_hello_world": {
            "type": "basilisk",
            "main": "src/main.py"
        }
    }
...
```bash
dfx canister call basilisk_hello_world get_message
```
...
```bash
dfx canister call basilisk_hello_world set_message '("Hello World!")'
```
...
URLs:
  Backend canister via Candid interface:
    basilisk_hello_world: http://127.0.0.1:8000/?canisterId=ryjl3-tyaaa-aaaaa-aaaba-cai&id=rrkah-fqaaa-aaaaa-aaaaq-cai
```
