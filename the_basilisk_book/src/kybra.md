...
Basilisk is currently developed by [Demergent Labs](https://github.com/demergent-labs), a for-profit company with a [grant](https://dfinity.org/grants) from [DFINITY](https://dfinity.org/).
...
Basilisk and the IC provide unique benefits and drawbacks, and both are not currently suitable for all application use-cases.

The following information will help you to determine when Basilisk and the IC might be beneficial for your use-case.

### Benefits

Basilisk intends to be a full Python environment for the IC (a decentralized cloud platform), with support for all of the Python language and as many relevant libraries and APIs as possible.

One of the core benefits of Basilisk is that it allows Python developers to bring their skills to the IC.
...
For the time being, Basilisk source code is not verifiable for reasons explained in the [caveats section](./caveats.md#wasm-module-hash-now-less-useful).
...
It's important to note that both Basilisk and the IC are early-stage projects. The IC officially launched in May of 2021, and Basilisk reached beta in December of 2022.

#### Basilisk

Some of Basilisk's main drawbacks can be summarized as follows:
...
Basilisk reached beta in December of 2022. It's an immature project that may have unforeseen bugs and other issues. We're working constantly to improve it. We hope to get to a production-ready 1.0 in 2024. The following are the major blockers to 1.0:
...
Basilisk is limited to what the IC is capable of (i.e. no sockets, no threads, etc) and does not yet have C extension support. Our goal is to support as many libraries and APIs as possible over time.
