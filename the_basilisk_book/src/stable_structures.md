...
Basilisk currently provides one stable structure called `StableBTreeMap`. It's similar to a Python dictionary or a map that you'd find in most languages, and has most of the common operations you'd expect such as reading, inserting, and removing values.
...
```python
from basilisk import nat8, StableBTreeMap
...
```python
from basilisk import blob, nat64, Principal, Record, StableBTreeMap, Vec
