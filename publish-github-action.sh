...
echo -e "sleeping for 30 seconds to ensure basilisk==$VERSION is fully registered on PyPI"
...
do
    ...
    sed -E -i "s/(basilisk==)(.*)()/\1$VERSION\3/" requirements.txt
...
git add --all
git commit -am "basilisk-bot automated release $VERSION"
...
if [[ "$VERSION" == *"rc"* ]];
then
    gh release create "$VERSION" "$HOME/.config/basilisk/$VERSION/rust_python_stdlib.tar.gz" -t "$VERSION" --prerelease
else
    gh release create "$VERSION" "$HOME/.config/basilisk/$VERSION/rust_python_stdlib.tar.gz" -t "$VERSION"
fi
