//! CPython IC notify functions (dynamic, per-service).
    ...
    services
        ...
        .flat_map(|service| {
            service
                .methods
                .iter()
                .map(move |method| generate_notify_fn(service, method))
        })
        .collect()
}

fn generate_notify_fn(service: &Service, method: &Method) -> TokenStream {
    let fn_name = format_ident!("ic_notify_{}_{}", service.name, method.name);
    let real_fn_name = format_ident!("notify_{}_{}", service.name, method.name);
    ...
    let param_extractions: Vec<TokenStream> = method
        ...
        .map(|(index, param)| {
            ...
            let actual_index = (index + 1) as isize; // index 0 is canister_id
            quote! {
    ...
    quote! {
        unsafe extern "C" fn #fn_name(
            ...
            let notify_result = #real_fn_name(canister_id_principal, #params);
