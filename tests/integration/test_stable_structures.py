...
def test_canister1_set_and_get(canisters):
    ...
    call_canister(c1, "stable_map_0_insert", '(0 : nat8, "value0")', example_dir=EXAMPLE_DIR)
    raw = call_canister(c1, "stable_map_0_get", "(0 : nat8)", example_dir=EXAMPLE_DIR)
    assert "value0" in raw


def test_canister1_contains_key(canisters):
    c1 = canisters.get("canister1") or list(canisters.values())[0]
    raw = call_canister(c1, "stable_map_0_contains_key", "(0 : nat8)", example_dir=EXAMPLE_DIR)
    assert "true" in raw


def test_canister2_set_and_get(canisters):
    c2 = canisters.get("canister2") or list(canisters.values())[1]
    call_canister(c2, "stable_map_5_insert", '(5 : nat64, "value5")', example_dir=EXAMPLE_DIR)
    raw = call_canister(c2, "stable_map_5_get", "(5 : nat64)", example_dir=EXAMPLE_DIR)
    assert "value5" in raw


def test_canister3_set_and_get(canisters):
    c3 = canisters.get("canister3") or list(canisters.values())[2]
    call_canister(c3, "stable_map_10_insert", '("key10", 10 : nat64)', example_dir=EXAMPLE_DIR)
    raw = call_canister(c3, "stable_map_10_get", '("key10")', example_dir=EXAMPLE_DIR)
    assert "10" in raw
